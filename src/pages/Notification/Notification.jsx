...
const Notification = () => {
	...
	return (
			...
			<>
				<Header></Header>
				  <div className="flex flex-col w-full gap-8 px-5 text-white 2xs:px-10 md:justify-center md:flex-row" style={{fontFamily:"Poppins"}}>
            <div className="flex flex-col justify-center w-full gap-1 px-5 py-5 2xs:gap-2 md:w-81 bg-app-black rounded-xl justify-items-center md:h-90">
					  	{
					  		buttonList.map((button, idx) => {
					  			return (
					  				<ButtonWithSymbol key={idx} title={button.title} selected={buttonId === button.id} onClick={handleClick(button.id)}
                                      buttonStyle={"px-4 py-4"}  fontStyle={"2xs:text-base text-sm 2xs:font-medium font-normal"}/>
					  			)
					  		})
					  	}
					  </div>
